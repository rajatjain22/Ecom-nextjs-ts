...
export const getAllProducts = async (options?: any): Promise<any> => {
  try {
    ...
    const processedProducts = products.map((product: any) => {
      if (product?.variants && product.variants.length > 0) {
        // Calculate the total quantity from variants (using quantity_in_stock)
        const totalQuantity = product.variants.reduce(
          (sum: any, variant: any) => sum + (variant.quantity_in_stock || 0), // Sum quantity_in_stock from variants
          0
        );

        // Calculate the price range (lowest and highest price from variants)
      ...
      // If no variants, return product as is
      return product;
    });
